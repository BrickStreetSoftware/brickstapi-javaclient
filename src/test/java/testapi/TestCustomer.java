...
public class TestCustomer 
{
	...
	public boolean arraySetEquals(String[] s1, String[] s2)
	{
		// XXX assumes s1 and s2 are sets.
		// XXX this will not work: s1 = { 'a', 'a' }    (since it's not a set)
	...
	//
	// FETCH METHODS ARE NOW CALLED BY OTHER TEST METHODS THAT CREATE NEW CUSTOMER RECORDS
	//
			...
			throws AuthenticationException, ClientProtocolException, URISyntaxException, IOException
	{
		...
		//
		{
			String sCustId = Long.toString(c2.id);		
			testFetchCustomer(sCustId, c2.emailAddress, c2.altCustomerID);
			testFetchCustomerEmail(sCustId, c2.emailAddress, c2.altCustomerID);
			testFetchCustomerAltId(sCustId, c2.emailAddress, c2.altCustomerID);
		}
			...
			throws AuthenticationException, ClientProtocolException, URISyntaxException, IOException
	{
		...
		Assert.assertTrue(arraySetEquals(c2attr.preferenceValues, prefVals));
	...
	@Parameters({"multiaddressName"})
	@Test
	public void testAddCustomerWithMultiAddress(String multiaddressName) 
			throws AuthenticationException, ClientProtocolException, URISyntaxException, IOException
	{
		// lookup attribute
		Attribute attrDef = api.getAttribute(multiaddressName);
		Assert.assertNotNull(attrDef, "Attribute Not Found");
		Assert.assertEquals(attrDef.name, multiaddressName);
		String attrType = attrDef.type;
		...
		if ("preference".equals(attrType)) {
			// add a preference
			attr = c.getAttribute(multiaddressName);
			if (attr == null) {
				attr = new CustomerAttribute();
				attr.name = attrDef.name;
				attr.type = attrDef.type;
			...
			// add a channel address
			attr = c.getChannelAddress(multiaddressName);
			if (attr == null) {
				attr = new CustomerAttribute();
				attr.name = attrDef.name;
				attr.type = attrDef.type;
		...
		CustomerAttribute c2attr = null;
		if ("preference".equals(attrType)) {
			c2attr = c2.getAttribute(multiaddressName);
		}
		else if ("multiaddress".equals(attrType)) {
			c2attr = c2.getChannelAddress(multiaddressName);
		}
		Assert.assertNotNull(c2attr);
		Assert.assertTrue(arraySetEquals(c2attr.preferenceValues, prefVals));
